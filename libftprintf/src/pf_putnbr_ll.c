...
static void	dot_flag(long long nb, t_format *f)
{
	int	size;

	size = ft_numlen(nb, 10);
	f->rtn += size;
...
static void	minus_flag(long long nb, t_format *f)
{
	int	size;
...
static void	zero_flag(long long nb, t_format *f)
{
	int	size;
...
static void	other_flag(long long nb, t_format *f)
{
	int	size;
...
void		pf_putnbr_ll(long long nb, t_format *f)
{
	...
	if (f->pred < ft_numlen(nb, 10))
		f->pred = ft_numlen(nb, 10);
	if (f->flag % fl2pn('.') == 0)
		dot_flag(nb, f);
