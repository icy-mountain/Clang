...
static void	dot_flag(unsigned long long nb, char *base, t_format *f)
{
	int	size;

	size = ft_numlen_ull(nb, 16);
	f->rtn += size;
	if (f->flag % fl2pn('#') == 0)
		f->pad -= 2;
...
static void	minus_flag(unsigned long long nb, char *base, t_format *f)
{
	int	size;
...
static void	zero_flag(unsigned long long nb, char *base, t_format *f)
{
	int	size;
...
static void	other_flag(unsigned long long nb, char *base, t_format *f)
{
	int	size;
...
void		pf_putnbr_base_ull(unsigned long long nb, char *base, t_format *f)
{
	...
	if (f->pred < ft_numlen_ull(nb, 16))
		f->pred = ft_numlen_ull(nb, 16);
	if (f->flag % fl2pn('.') == 0)
		dot_flag(nb, base, f);
